...
def answer_question(user_id: int, mode: str, question: str) -> str:
    ...
    "  short bullet points for that part. CRITICAL: each bullet MUST start on\n"
    "  its own new line using markdown list syntax ('- **Label** — detail'),\n"
    "  never run multiple bullets together inline in one paragraph separated\n"
    "  only by dashes.\n"
    "- For emotional or conversational replies, keep it natural prose instead\n"
    "  of bullets.\n"
    "- Keep the ENTIRE reply short: 1-2 sentences of reaction, then at most\n"
    "  3-4 bullets if guidance is needed. Do not write long paragraphs of\n"
    "  explanation before or between bullets — this is a chat app, not an\n"
    "  essay.\n\n"
    ...
    "  a detached quote.\n"
    "- CRITICAL DISTINCTION — when relevant personal log data IS available:\n"
    "  do NOT fall back to a generic tip or generic explanation. Build your\n"
    "  answer around their SPECIFIC pattern — name the actual habit, the actual\n"
    "  day, the actual recurrence you can see in their logs, and explain the\n"
    "  likely cause in terms of THAT specific pattern. 'You've missed Fajr three\n"
    "  Mondays in a row, right after you noted staying up late gaming' is what\n"
    "  personalized looks like; 'try going to bed earlier' is not — avoid that.\n"
    "  When NO relevant personal data is available for this question, that's\n"
    "  when general guidance is appropriate — but say it warmly and specifically\n"
    "  enough that it still doesn't feel like a canned tip.\n\n"
