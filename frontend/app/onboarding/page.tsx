    ...
    <div className="min-h-screen bg-[#F7F3EC] flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="w-full bg-white border border-stone-200 p-8 rounded-3xl shadow-sm">
          <button onClick={() => router.back()} className="text-stone-400 text-sm mb-5 flex items-center gap-1 hover:text-stone-600 transition-colors">
            <ArrowLeft size={14} /> Back
          </button>

          <div className="flex items-center gap-2 mb-4">
            <span className="w-1.5 h-1.5 rounded-full bg-[#3C6E7A]" />
            <span className="text-xs font-semibold tracking-widest text-stone-400 uppercase">{mode === "habit" ? "Habit tracker" : "Tazkiya"} onboarding</span>
          </div>

          <h1 className="text-3xl font-serif text-stone-900 mb-3 leading-tight">Let&apos;s get to know you.</h1>
          <p className="text-stone-500 mb-8">A few quick questions to start.</p>

          {questions.map((q, index) => (
            <div key={index} className="mb-5">
              <label className="block text-sm font-semibold text-stone-700 mb-2">{q}</label>
              <textarea className="bg-stone-100 border border-stone-200 p-3 w-full rounded-2xl focus:outline-none focus:border-[#3C6E7A] text-stone-800" rows={2} value={answers[index]} onChange={(e) => updateAnswer(index, e.target.value)} />
            </div>
          ))}

          <button onClick={handleSubmit} className="bg-[#3C6E7A] hover:bg-[#2C5560] text-white px-4 py-3 rounded-full w-full font-semibold mt-2 transition-colors">
            Continue
          </button>
        </div>
      </div>
