  ...
  return (
    <div className="max-w-3xl mx-auto p-5 sm:p-8">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-serif text-[#1E2A32]">Habits Tracker</h1>
          <p className="text-sm text-[#5A6B7A] mt-1">{doneCount} of {habits.length} done today</p>
        </div>
        <button onClick={() => { resetForm(); document.getElementById("habit-name-input")?.focus(); }} className="flex items-center gap-2 bg-[#2E5E4E] hover:bg-[#254D40] text-white px-5 py-2.5 rounded-full font-semibold text-sm transition-colors">
          <Plus size={16} /> Add habit
        </button>
      </div>

      <div className="bg-white border border-[#E5E0D5] rounded-2xl p-6 mb-5">
        <p className="text-sm font-semibold text-[#1E2A32] mb-4">{editingId ? "Edit habit" : "New habit"}</p>
          ...
          return (
            <div key={habit.id} className={`flex items-center justify-between p-4 rounded-2xl border mb-2.5 transition-colors ${isDone ? "border-[#2E5E4E] bg-[#2E5E4E]/5" : "border-[#E5E0D5] bg-white"}`}>
              <button onClick={() => toggleDone(habit.id, habit.name)} className="flex items-center gap-3 flex-1 text-left">
                ...
                <span className="text-sm text-[#1E2A32]">{habit.name}</span>
              </button>
              <div className="flex gap-3 text-[#C9C4B8]">
                <button onClick={() => handleEdit(habit)} className="hover:text-[#2E5E4E]"><Pencil size={15} /></button>
                <button onClick={() => handleDelete(habit.id)} className="hover:text-[#E0674F]"><Trash2 size={15} /></button>
              </div>
            </div>
          );
        })
