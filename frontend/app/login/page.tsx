...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    setError("");
    try {
      const res = await fetch("https://occupier-squall-handmade.ngrok-free.dev/login", {
        method: "POST",
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <div className="w-full max-w-md bg-white/90 backdrop-blur-sm border border-[#E5E0D5] p-6 sm:p-8 rounded-3xl shadow-lg">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs font-semibold tracking-widest text-[#8A8478] uppercase">Welcome back</span>
          </div>

          <h1 className="text-3xl sm:text-4xl font-serif text-[#1E2A32] mb-3 leading-tight">Continue your rise.</h1>
          <p className="text-[#5A6B7A] mb-8">Sign in to pick up where you left off.</p>

          {error && <div className="bg-red-50 text-red-600 text-sm p-3 rounded-lg mb-4">{error}</div>}

          <label className="block text-sm font-semibold text-[#1E2A32] mb-2">Email or Username</label>
          <div className="relative mb-5">
            <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8A8478]" size={18} />
            <input className="bg-[#F4F1EA] border border-[#E5E0D5] pl-11 pr-4 py-3 w-full rounded-full focus:outline-none focus:border-[#2E5E4E] text-[#1E2A32]" value={identifier} onChange={(e) => setIdentifier(e.target.value)} />
          </div>
          ...
          <button onClick={handleLogin} className="bg-[#2E5E4E] hover:bg-[#254D40] text-white px-4 py-3 rounded-full w-full font-semibold transition-colors">
            Sign in
          </button>

          <p className="text-center text-sm text-[#5A6B7A] mt-6">
            Don&apos;t have an account? <a href="/signup" className="font-semibold text-[#2E5E4E]">Sign up</a>
          </p>
          <button onClick={handleGuest} className="w-full text-center text-sm font-medium underline text-[#2E5E4E]/70 mt-3">
            Continue as guest
          </button>
        </div>
      </div>
